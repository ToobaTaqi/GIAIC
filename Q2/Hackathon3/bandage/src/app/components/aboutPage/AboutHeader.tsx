import React from "react";
import { globalImages } from "@/app/assets";
import Image from "next/image";

export default function AboutHeader() {
  return (
    <section className="flex flex-col justify-center items-center gap-[40px] text-center">
      <div className="flex flex-col gap-[40px]">
        <h2 className="font-bold text-4xl ">ABOUT US</h2>
        <p className="text-[#737373]">
          We know how large <br className="lg:hidden" /> objects will act, but
          things <br className="lg:hidden" /> on a small scale just do
          <br className="lg:hidden" /> not act that way.
        </p>
        <button className="bg-[#23A6F0] rounded py-[15px] px-[40px] text-sm font-bold text-white">
          Get Quote Now
        </button>
      </div>
      <Image
        src={globalImages.aboutgirl}
        alt=""
        width={100}
        height={100}
        className="w-[387px] h-[440px] min-w-[387px] min-h-[440px]"
      />
    </section>
  );
}
