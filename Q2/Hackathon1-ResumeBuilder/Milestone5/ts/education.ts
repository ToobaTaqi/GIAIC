const addNewEducation = document.getElementById(
  "addNewEducation"
...
addNewEducation.addEventListener("click", (e) => {
  ...
  const eduUl = document.getElementById("eduFieldSets") as HTMLUListElement;
  ...
  if (eduUl) {
    ...
    const deleteEdu = newLi.querySelector(".deleteEdu") as HTMLButtonElement;
...
const educationSaved = document.getElementById(
  "educationSaved"
) as HTMLButtonElement;
...
educationSaved.addEventListener("click", (e) => {
  ...
  const institutes = document.querySelectorAll(
    ".institute"
  ) as NodeListOf<HTMLInputElement>;
  ...
  const eduUl = document.getElementById("eduUl") as HTMLUListElement;

  let allFieldsFilled = true;

  for (let i = 0; i < qualifications.length; i++) {
    if (
      !qualifications[i].value.trim() ||
